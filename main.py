...
class myNet(nn.Module):
    def __init__(self, inDim):
        super(myNet,self).__init__()                    # 直接复制
...
def train_val(model, trainloader, valloader,optimizer, loss, epoch, device, save_):
    ...
    for i in range(epoch):                 # 训练epoch 轮
        ...
        model.train()                         # 模型设置为训练状态（因为有些参数只在训练起作用，在验证时候不起作用）      结构
        train_loss = 0.0
        val_loss = 0.0
        for data in trainloader:                     # 从训练集取一个batch的数据
            optimizer.zero_grad()                   # 梯度清0
            x = data[0].to(device)                  # 将数据放到设备上
...
optimizer = optim.SGD(model.parameters(), lr=0.001, momentum=0.9)             # 定义优化器  动量 自带的sgd
